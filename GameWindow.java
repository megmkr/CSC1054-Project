...
public class GameWindow extends JPanel{
   ...
   public class MovingTimer implements ActionListener{
      ...
      boolean victory = false;  
         //getting all objects within one block radius of player to check for collision
            ...
            for(int j=0; j<gameObjectArray.get(0).size(); j++){
               if(gameObjectArray.get(i).get(j) != null && 
                  ...
                  if(gameObjectArray.get(i).get(j).getColor()== Color.RED){
                     int input = JOptionPane.showOptionDialog(null, "YOU WON!", "Victory Screen", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE, null, null, null);
